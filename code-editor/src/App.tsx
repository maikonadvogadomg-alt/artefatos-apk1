...
function App() {
  ...
  if (showExtrator) {
    return <Extrator onBack={() => setShowExtrator(false)} />;
  }
  ...
  if (!currentProject) {
    return (
      <div style={filterStyle}>
        {toggleBtn}
        <input ref={zipInputRef} type="file" onChange={handleZipFile} className="hidden" />
        <TemplateSelector
          projects={projects}
          ...
          onOpenExtrator={() => setShowExtrator(true)}
        />
      </div>
    );
  }
