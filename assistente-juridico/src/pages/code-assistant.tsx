...
export default function CodeAssistant() {
  ...
  const voiceChatSend = useCallback(async (userText: string) => {
    ...
    try {
      const { directStream } = await import("@/lib/ai-direct");
      const systemPrompt = `Você é um assistente inteligente versátil. Responda com precisão, clareza e profundidade em português.`;
      const chatMsgs = [
        { role: "system" as const, content: systemPrompt },
        ...msgs.map(m => ({ role: m.role === "assistant" ? "assistant" as const : "user" as const, content: m.text })),
      ];
      let fullText = "";
      await directStream(chatMsgs, (chunk) => { fullText += chunk; }, {
        key: apiKey.trim(), url: apiUrl.trim().replace(/\/$/, ""), model: apiModel.trim(),
      });
      const reply = fullText.trim() || "Desculpe, não consegui responder.";
      setVoiceMsgs(prev => [...prev, { role: "assistant", text: reply }]);
      playTts(reply);
    } catch (err: any) {
      setVoiceMsgs(prev => [...prev, { role: "assistant", text: `Erro: ${err?.message || "Tente novamente."}` }]);
    } finally {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      const { directStream } = await import("@/lib/ai-direct");
      const systemPrompt = `Você é um assistente inteligente versátil. Responda com precisão, clareza e profundidade.\n\nREGRAS:\n1. Responda de forma completa e direta\n2. Quando gerar código, use boas práticas e retorne o código INTEIRO\n3. Responda em português, salvo se o usuário escrever em outro idioma\n4. Não há restrição de assunto — responda perguntas jurídicas, técnicas, gerais ou qualquer tema`;
      const chatMsgs = [
        { role: "system" as const, content: systemPrompt },
        ...newHistory.map(h => ({ role: h.role === "assistant" ? "assistant" as const : "user" as const, content: h.content })),
      ];
      let fullText = "";
      await directStream(chatMsgs, (chunk) => {
        fullText += chunk;
        setResult(fullText);
      }, { key: apiKey.trim(), url: apiUrl.trim().replace(/\/$/, ""), model: apiModel.trim(), signal: controller.signal });
